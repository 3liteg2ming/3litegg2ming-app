...
function mergeRows(existing: ExtractedPlayerStatRow, incoming: ExtractedPlayerStatRow): ExtractedPlayerStatRow {
  const merged: ExtractedPlayerStatRow = {
    ...existing,
    playerId: text(existing.playerId) || text(incoming.playerId) || null,
    aflPlayerId: existing.aflPlayerId ?? incoming.aflPlayerId ?? null,
    name: existing.name || incoming.name,
    team: existing.team || incoming.team || null,
    goals: null,
    behinds: null,
    disposals: null,
    kicks: null,
...
function rowsMatch(left: ExtractedPlayerStatRow, right: ExtractedPlayerStatRow): boolean {
  ...
  if (left.team && right.team && left.team !== right.team) return false;
  if (compactNameKey(left.name) && compactNameKey(left.name) === compactNameKey(right.name)) return true;
...
function goalKickerToRow(value: unknown, team: ExtractedPlayerStatTeam): ExtractedPlayerStatRow | null {
  ...
  const name = normalizeName(row.name ?? value);
  if (!name) return null;
...
export function mergeExtractedPlayerStats(
  ...
  for (const incoming of incomingRows) {
    const matchIndex = merged.findIndex((row) => rowsMatch(row, incoming));
    if (matchIndex === -1) {
...
export function findBestRosterPlayerMatch(
  ...
  const lookupName = normalizeName(sourceRow.name);
  ...
  const lookupExactKeys = new Set([normalizeNameKey(lookupName), compactNameKey(lookupName)]);
  const exactCandidates = roster.filter((candidate) => {
    ...
    return nameVariantKeys(candidateName(candidate)).some((key) => lookupExactKeys.has(key));
  });
...
export function fantasyValueForRow(row: ExtractedPlayerStatRow): number | null {
  return row.fantasyPoints ?? row.afl_fantasy ?? null;
}

export function mergeIncomingOverExisting(existing: ExtractedPlayerStatRow, incoming: ExtractedPlayerStatRow): ExtractedPlayerStatRow {
  const merged: ExtractedPlayerStatRow = {
    ...existing,
    playerId: text(incoming.playerId) || text(existing.playerId) || null,
    aflPlayerId: incoming.aflPlayerId ?? existing.aflPlayerId ?? null,
    name: incoming.name || existing.name,
    team: incoming.team || existing.team || null,
    goals: incoming.goals ?? existing.goals,
    behinds: incoming.behinds ?? existing.behinds,
    disposals: incoming.disposals ?? existing.disposals,
    kicks: incoming.kicks ?? existing.kicks,
    handballs: incoming.handballs ?? existing.handballs,
    marks: incoming.marks ?? existing.marks,
    tackles: incoming.tackles ?? existing.tackles,
    hitouts: incoming.hitouts ?? existing.hitouts,
    afl_fantasy: incoming.afl_fantasy ?? incoming.fantasyPoints ?? existing.afl_fantasy ?? existing.fantasyPoints,
    fantasyPoints: incoming.fantasyPoints ?? incoming.afl_fantasy ?? existing.fantasyPoints ?? existing.afl_fantasy,
  };

  const fantasy = fantasyValueForRow(merged);
  merged.afl_fantasy = fantasy;
  merged.fantasyPoints = fantasy;
  return merged;
}

export function hasRealPlayerId(candidate: RosterPlayerMatchCandidate | ExtractedPlayerStatRow): boolean {
  const value = text((candidate as { id?: string | null; playerId?: string | null }).id || (candidate as { playerId?: string | null }).playerId);
