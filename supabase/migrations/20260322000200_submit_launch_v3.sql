-- Submit Launch V3: Fixed screenshot slots + team stats + quarter scores
-- 1) Extend stat_key constraint to include goals, behinds, afl_fantasy
-- 2) Update eg_submit_result_v2 to read per-screenshot slot metadata
-- 3) Store team stats and quarter scores from p_ocr payload

begin;

-- ---------------------------------------------------------------------------
-- A) Extend stat_key constraint on eg_fixture_submission_images
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- B) Replace eg_submit_result_v2
--    Changes:
--    - Read imageType / statKey / pageNumber from each screenshot element
--    - Store p_ocr -> 'teamStats' into ocr_team_stats
--    - Store p_ocr -> 'quarterScores' as text into ocr_raw_text
--    - Set ocr_status to 'pending' (not 'done') for screenshots awaiting admin OCR
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Insert per-slot screenshot metadata (reads imageType/statKey/pageNumber from each element)
