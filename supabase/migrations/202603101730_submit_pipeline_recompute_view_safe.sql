...
as $$
begin
  -- Live stats surfaces now read from eg_fixture_player_stats / season-total views.
  -- Keep submit non-blocking even if legacy materialization tables or views differ.
  return;
end;
$$;
...
begin
  ...
  values (
    ...
    p_ocr ->> 'rawText',
    coalesce(p_ocr -> 'teamStats', '{}'::jsonb),
    case
      when p_ocr is null then '{}'::jsonb
      else jsonb_build_object('playerLines', coalesce(p_ocr -> 'playerLines', '[]'::jsonb))
    end,
    p_notes,
    v_submitted_at
  );
