...
-- HOTFIX 2026-04-30:
--   1. eg_submit_result_v2 was crashing with `function max(uuid) does not exist`
--      (SQLSTATE 42883) on submissions that included goal kickers. Postgres has
--      no aggregate `max(uuid)`, so the goal-kicker identity CTE fails. Cast
--      team_id to text for aggregation and back to uuid on output.
--   2. eg_admin_set_user_role_and_team now also syncs team_id into the legacy
--      public.profiles table so older surfaces that still read profiles stay
--      in sync. eg_profiles remains the source of truth.

-- ---------------------------------------------------------------------------
-- 1. eg_submit_result_v2: same logic as 20260430120000 but with safe uuid
--    aggregation. Deadline / admin-approval gate stays removed. Screenshots
--    still required. Both home and away coaches can submit. Status updates
--    to FINAL. Recompute hooks preserved.
-- ---------------------------------------------------------------------------
...
begin
  ...
  identity_rows as (
    select
      ...
      -- Postgres has no aggregate max(uuid). Cast to text to aggregate, cast back.
      max(team_id::text)::uuid as team_id
    from kicker_rows
...
-- ---------------------------------------------------------------------------
-- 2. eg_admin_set_user_role_and_team: keep upsert-into-eg_profiles behaviour
--    from 20260428120000, and additionally sync team_id into the legacy
--    public.profiles table so older read paths stay consistent.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Best-effort sync into legacy public.profiles if it exists. Silent on
  -- missing column / table so we never break the primary path.
  if to_regclass('public.profiles') is not null then
    begin
      execute 'update public.profiles set team_id = $1 where user_id = $2'
        using p_team_id, p_user_id;
    exception when undefined_column then
      begin
        execute 'update public.profiles set team_id = $1 where id = $2'
          using p_team_id, p_user_id;
      exception when undefined_column then
        null;
      end;
