-- Fix team stats pipeline: store team stats + quarter scores directly on eg_fixtures
-- so Match Centre can read them without needing submissions table access.
-- Also add SELECT RLS policy on submissions for authenticated users.

begin;

-- ---------------------------------------------------------------------------
-- A) Add team_stats_json and quarter_scores_json columns to eg_fixtures
--    These store the coach-submitted team stats and quarter progression
--    directly on the fixture row, ensuring downstream readers can access them
--    without needing to query the submissions table.
-- ---------------------------------------------------------------------------
do $$
begin
  if not exists (
    ...
    where table_schema = 'public'
      ...
      and column_name = 'team_stats_json'
  ) then
    alter table public.eg_fixtures add column team_stats_json jsonb default null;
  end if;
  ...
  if not exists (
    ...
    where table_schema = 'public'
      and table_name = 'eg_fixtures'
      and column_name = 'quarter_scores_json'
  ) then
    alter table public.eg_fixtures add column quarter_scores_json jsonb default null;
  end if;
end;
$$;

-- ---------------------------------------------------------------------------
-- B) Ensure submissions table has RLS with a SELECT policy for authenticated
-- ---------------------------------------------------------------------------
do $$
begin
  ...
  -- Drop existing policy if present to avoid conflict
  drop policy if exists "authenticated_read_submissions" on public.submissions;
...
exception when others then
  raise notice 'RLS policy setup for submissions: %', sqlerrm;
end;
$$;

-- ---------------------------------------------------------------------------
-- C) Replace eg_submit_result_v2 to also store team stats + quarter scores
--    directly on the eg_fixtures row
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_quarter_scores_jsonb jsonb := null;
...
begin
  ...
  -- Extract screenshots array from p_ocr
  ...
  -- Only the home team coach may submit
  if v_team_id::text <> coalesce(v_fixture_row.home_team_id::text, '') then
    raise exception 'Only the home team coach can submit this fixture result';
  end if;
  ...
  -- Insert per-slot screenshot metadata
  ...
  -- Update fixture with scores AND team stats + quarter scores directly
  ...
  set
    ...
    team_stats_json = v_team_stats,
    quarter_scores_json = v_quarter_scores_jsonb
  where id = p_fixture_id;

  -- Upsert goal kicker player records
  with kicker_rows as (
    select
      case
...
-- ---------------------------------------------------------------------------
-- D) Backfill: copy team stats + quarter scores from existing submissions
--    to their corresponding eg_fixtures rows, so Match Centre can read them
-- ---------------------------------------------------------------------------
update public.eg_fixtures f
set
  team_stats_json = s.ocr_team_stats,
  quarter_scores_json = case
    when s.ocr_raw_text is not null and s.ocr_raw_text ~ '^\{' then s.ocr_raw_text::jsonb
    else f.quarter_scores_json
  end
from public.submissions s
where s.fixture_id = f.id
  and f.team_stats_json is null
  and s.ocr_team_stats is not null
  and jsonb_typeof(s.ocr_team_stats) = 'object'
  and s.ocr_team_stats <> '{}'::jsonb;
